...
class WsseWithExternalSignerMiddleware extends Middleware
{
     ...
     * @var bool
     */
    private $useExternalXmlSigner = false;

    /**
     * @var \RobRichards\WsePhp\WSSEExternalXmlSignerInterface
     */
    private $externalXmlSigner;

    /**
     * @var array Strings with node names that should be signed by external signer
     */
    private $nodesForExtSignature;
    ...
    {
        return 'wsse_with_external_signer_middleware';
    }
    ...
    public function withExternalSigner(WSSEExternalXmlSignerInterface $externalXmlSigner, array $nodesForExtSignature = []): self
    {
        $this->useExternalXmlSigner = true;
        $this->nodesForExtSignature = $nodesForExtSignature;
        $this->externalXmlSigner = $externalXmlSigner;
    ...
    public function beforeRequest(callable $handler, RequestInterface $request): Promise
    {
        ...
        if ($this->useExternalXmlSigner) {
            $wsse->registerExternalSigner($this->externalXmlSigner);
            $wsse->signSoapPartExt($this->nodesForExtSignature);
        } else {
